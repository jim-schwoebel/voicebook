...
for i in range(len(listdir)):
    try:
        if listdir[i][-5:] not in ['Store','.json']:
            ...
            if filename[0:-4]+'_tpotaudio.json' not in listdir:
                ...
                for j in range(len(model_list)):
                    ...
                    if modelname.find('regression')>0:
                        if float(output)<0.5:
                            classname=name1
                        else:
                            classname=name2
                    ...
                    modeltypes.append(g['model type'])

                os.chdir(load_dir)

                jsonfilename=filename[0:-4]+'_tpotaudio.json'
