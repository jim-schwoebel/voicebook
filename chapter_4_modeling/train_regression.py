...
try:
	...
	poly_nn= Perceptron(fit_intercept=False, max_iter=10, tol=None,
	                 shuffle=False).fit(X_train2, y_train)
	predictions = cross_val_predict(poly_nn, X_test2, y_test, cv=6)
	accuracy = metrics.r2_score(y_test, predictions)
	f=open('poly_nn.pickle','wb')
	pickle.dump(poly_nn,f)
	f.close()
except:
...
varnames=['ols.pickle', 'ridge.pickle', 'lasso.pickle', 'enet.pickle', 'lars.pickle', 
		  ...
		  'ransac.pickle', 'theilsen.pickle', 'huber.pickle', 'poly_nn.pickle']
...
print('%s has the lowest mean absolute error (%s)'%(modeltypes[i], str(minval)))
# rename file 
os.chdir(tempdir)
newname= foldername+'.pickle'
print('saving file to disk (%s)...'%(newname))
os.rename(varnames[i], newname)
...
data={
	'model name':newname,
	'model type':modeltypes[ind-1],
	'stats':{
			'explained_variance':explained_variances[ind-1],
			'mean_absolute_error':mean_absolute_errors[ind-1],
			#'mean_squared_error': mean_squared_errors[ind-1],
			'mean_squared_log_error':mean_squared_log_errors[ind-1],
			'median_absolute_error':median_absolute_errors[ind-1],
